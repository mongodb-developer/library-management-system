 ...
 * Routes
 *
 * GET /reservations/admin: Returns a list of reservations.
...
reservations.get('/admin', protectedRoute, adminRoute, async (req: AuthRequest, res) => {
    const limit = parseInt(req?.query?.limit as string) || undefined;
    const skip = parseInt(req?.query?.skip as string) || undefined;

    const reservations = await issueDetailsController.getRecentReservations(limit, skip);
